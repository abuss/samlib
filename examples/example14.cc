...
public:
  ...
  {
    workers.push_back( this->environment.template make_agent<Tin>(signal_task(this->task, &free_workers, 0)) );
    free_workers.enqueue(0);

    workers.push_back( this->environment.template make_agent<Tin>(signal_task(this->task, &free_workers, 1)) );
    free_workers.enqueue(1);

    workers.push_back( this->environment.template make_agent<Tin>(signal_task(this->task, &free_workers, 2)) );
...
struct engine
{
  ...
  engine(Gen _generator, Sink _sink)
  {
    ...
    p2 = env.create_agent<processing_agent<env_t,int>>(my_transform(peek,p3));
